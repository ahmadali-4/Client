...
const FriendElement = ({ img, firstName, lastName, online, _id }) => {
  const handleStartConversation = () => {
    socket?.emit("start_conversation", { to: _id, from: user_id }, (data) => {
      console.log(data); // Optional: Handle any callback data returned from the server
    });
  };
  const theme = useTheme();
